...
const MapContainer = ({ theme }: MapContainerProps) => {
  ...
  useEffect(() => {
    ...
    if (!mapRef.current) {
      ...
      // ⚡ Bolt: Only update style when theme changes, preventing expensive reloads on data fetch
  ...
  }, [theme]); // ⚡ Bolt: Remove `pois` dependency so map doesn't reload style when data arrives

  useEffect(() => {
    // ⚡ Bolt: Separate effect strictly for updating data without touching the map style
